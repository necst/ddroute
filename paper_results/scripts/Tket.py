...
from pytket.passes import SequencePass, RoutingPass
...
architecture = Architecture(coupling_map.get_edges())

#Standard gate set
gate_set = ['cx','rz','sx','x']

tk_circuit = circuit_from_qasm(sys.argv[1])

#Route the circuit
ts = time.time()
cu = CompilationUnit(tk_circuit, [ConnectivityPredicate(architecture)])
passes = SequencePass([RoutingPass(architecture)])
passes.apply(cu)
te = time.time()
routed_qc = transpile(tk_to_qiskit(cu.circuit), basis_gates=gate_set, optimization_level = 0)
...
print(f'TKET,{os.path.basename(sys.argv[1])},{os.path.basename(sys.argv[2])},{num_qubits},{init_depth},{initial_operations},{initial_cx},{final_depth},{final_operations},{final_cx},{swap_count},{sys.argv[3]},{route_time}',end="")